...
struct DynamicStylingExample: View {
    ...
    @State var show3DModels = false // 3D 모델 표시 여부를 제어하는 옵션 추가
    ...
    var body: some View {
        Map(viewport: $viewport) {
            ...
            // ModelLayer를 사용하지 않는 대체 방식의 ModelsComponent 사용
            if show3DModels {
                SimpleModelMarkersComponent()
            }
        }
        .mapStyle(mapStyle)
    ...
    private var settingsBody: some View {
        VStack(alignment: .leading) {
            ...
            Toggle("Show Model Markers", isOn: $show3DModels)
        }
        .padding(10)
...
/// 기본 SymbolLayer를 사용하여 3D 모델 대신 마커로 표시하는 단순화된 구현
struct SimpleModelMarkersComponent: MapStyleContent {
    var body: some MapStyleContent {
        // GeoJSONSource를 생성하여 모델 위치 데이터 제공
        GeoJSONSource(id: "models-markers-source")
            .data(.featureCollection(modelLocations))
            
        // 모델 위치에 표시할 아이콘 이미지 정의
        CircleLayer(id: "model-marker-circles", source: "models-markers-source")
            .circleRadius(10)
        ...
        // 모델 이름을 표시하는 심볼 레이어
        SymbolLayer(id: "model-marker-labels", source: "models-markers-source")
            .textField(Exp(.get) { "model" })
            .textSize(12)
            .textAnchor(.top)
            //.textOffset([0, 1.5])
    ...
    var modelLocations: FeatureCollection {
