...
struct ClusteringExample: View {
    ...
    // Define DC coordinates explicitly
    private let dcCoordinates = CLLocationCoordinate2D(latitude: 38.9072, longitude: -77.0369)

    var body: some View {
        MapReader { proxy in
            Map(initialViewport: .camera(center: dcCoordinates, zoom: 10))
                .mapStyle(.dark)
                .onStyleLoaded { _ in
