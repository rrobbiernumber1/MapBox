...
private struct FadingCircle: MapStyleContent {
    var body: some MapStyleContent {
        ...
        FillExtrusionLayer(id: "fill-layer-id", source: "source-id")
            // @_spi 보호 레벨 속성 제거하고 접근 가능한 속성으로 대체
            .fillExtrusionColor(UIColor(red: 0, green: 0, blue: 0.8, alpha: 0.3)) // 약한 파란색으로 대체
            .fillExtrusionOpacity(0.7)
            .fillExtrusionHeight(100000) // 높이 추가
            .fillExtrusionBase(0)
            .fillExtrusionAmbientOcclusionIntensity(0.3) // 그림자 효과 추가
            .fillExtrusionAmbientOcclusionRadius(5)
    }
}
