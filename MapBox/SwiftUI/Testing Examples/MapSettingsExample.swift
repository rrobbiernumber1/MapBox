    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            Map(initialViewport: .camera(center: .berlin, zoom: 12))
                ...
                // 오류 발생하는 기능 제거
                // .collectPerformanceStatistics(...) 제거
                ...
                // cullingShape 관련 기능 제거
                // .cullingShape(settings.cullingShape) 제거
                .ignoresSafeArea()
                .sheet(isPresented: $settingsOpened) {
                    SettingsView(settings: $settings)
                        .defaultDetents()
                }
                // alignment 오류 수정 - 명시적으로 SwiftUI.Alignment 사용
                .overlay(alignment: SwiftUI.Alignment.trailing) {
