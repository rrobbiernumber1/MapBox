...
final class DistanceExpressionExample: UIViewController, ExampleProtocol {
    ...
    func circleRadius(forZoom zoom: CGFloat) -> Double {
        ...
        // 수정: Projection.metersPerPoint 대신 metersPerPointAtLatitude 사용
        let metersPerPoint = metersPerPointAtLatitude(latitude: centerLatitude, zoom: Double(zoom))
    ...
    // 수정: 미터/픽셀 변환 계산 함수 추가
    private func metersPerPointAtLatitude(latitude: Double, zoom: Double) -> Double {
        // 지구 원주(equator)의 길이 (미터)
        let earthCircumference: Double = 40075016.686
        
        // 위도에 따른 보정 (위도가 높을수록 경도 1도당 거리가 줄어듦)
        let latitudeRadians = latitude * .pi / 180.0
        let latitudeCorrection = cos(latitudeRadians)
        
        // 줌 레벨에 따른 보정 (줌 레벨이 높을수록 1픽셀당 미터가 줄어듦)
        let zoomCorrection = pow(2.0, zoom)
        
        // 최종 미터/픽셀 계산
        return earthCircumference / (256 * zoomCorrection) * latitudeCorrection
    }
}
