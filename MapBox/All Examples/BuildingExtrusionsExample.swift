...
final class BuildingExtrusionsExample: UIViewController, ExampleProtocol {
    ...
    internal func addBuildingExtrusions() throws {
        ...
        var layer = FillExtrusionLayer(id: "3d-buildings", source: "composite")
            ...
            // 제거: .fillExtrusionVerticalScale(Exp(.interpolate, Exp(.linear), Exp(.zoom), 15, 0, 15.05, 1))
        ...
        // fillExtrusionLineWidth도 @_spi로 보호되어 있어 접근 불가
        var wallsOnlyExtrusionLayer = layer
        // .fillExtrusionLineWidth(2) 제거
    ...
    // 기존 열거형 대신 간단한 문자열 상수 사용
    private enum AlignmentMode: String {
        case flat = "flat"
        case terrain = "terrain"
    }
    
    private var baseAlignment: AlignmentMode = .flat
    private var heightAlignment: AlignmentMode = .flat
    ...
    @objc private func baseAlignmentButtonTapped(_ sender: UIButton) {
        ...
        */
        ...
        // 대신 사용자에게 알림
        showAlignmentChangeAlert(for: "base", mode: baseAlignment.rawValue)
    }
    ...
    @objc private func heightAlignmentButtonTapped(_ sender: UIButton) {
        ...
        */
        ...
        // 대신 사용자에게 알림
        showAlignmentChangeAlert(for: "height", mode: heightAlignment.rawValue)
    }
    
    private func showAlignmentChangeAlert(for alignmentType: String, mode: String) {
        let alert = UIAlertController(
            title: "Alignment Change",
            message: "\(alignmentType.capitalized) alignment changed to '\(mode)', but the property is inaccessible due to @_spi protection level",
